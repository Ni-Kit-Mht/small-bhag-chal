...
function createGameBoard() {
    ...
    if(playerType === 'bhakra'){
        squareIndices.forEach(square => {
            if(gameBoard.children[square].style.backgroundColor === bhakraColor){
                gameBoard.children[square].style.boxShadow = `0 0 20px rgba(45, 13, 158, 0.8)`;
            }
        });
      }else{
        squareIndices.forEach(square => {
            if(gameBoard.children[square].style.backgroundColor === bhagColor){
                gameBoard.children[square].style.boxShadow = `0 0 20px rgba(45, 13, 158, 0.8)`;
            }
        });
...
function getSurroundingSquares(clickedSquareIndex) {
    ...
    let counter = 0; // Declare counter outside the function only if it's meant to persist across calls

    function handleSquareClick(event, clickedSquareIndex, cell) {
        
        squareIndices.forEach(square => {
            if( gameBoard.children[clickedSquareIndex].style.backgroundColor === bhagColor || gameBoard.children[clickedSquareIndex].style.backgroundColor === bhakraColor){
                gameBoard.children[square].style.boxShadow = ""; // Remove glow
            }
        });

        counter = 0; // Reset counter for each click
        
        squareIndices.forEach(square => {
            if (gameBoard.children[square].style.backgroundColor === bhakraColor) {

                counter += 1; // Increment counter if the color matches
            }
        });
        
        if (counter < 3) {
            showGameOverMessage(); // Call the game over logic
        }
        ...
        if (previousClick !== null && gameBoard.children[previousClick].style.backgroundColor === bhagColor && firstSquareColor === bhagColor) {
            if (previousClick === 13) {
        ...
        if (firstClickedSquare) {
            ...
            playerType = playerType === 'bhag' ? 'bhakra' : 'bhag';
            if(playerType === 'bhakra'){
                squareIndices.forEach(square => {
                    if(gameBoard.children[square].style.backgroundColor === bhakraColor){
                        gameBoard.children[square].style.boxShadow = `0 0 20px rgba(45, 13, 158, 0.8)`;
